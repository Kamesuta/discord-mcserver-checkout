...
詳細なコマンド仕様は [docs/コマンド.md](docs/コマンド.md) を参照してください。

### 一般ユーザー向けコマンド (`/mcserver`)
- `checkout` - サーバー貸出申請
- `return` - 自分のサーバーを返却
- `extend` - 貸出期限を環境変数で指定した日数だけ延長
...
- `CHECKOUT_DEFAULT_PERIOD_DAYS`: イベント日未指定時のデフォルト貸出期間
- `CHECKOUT_EXTEND_DAYS`: `/mcserver extend` と延長ボタンで加算する日数
